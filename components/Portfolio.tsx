        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {cases.map((item, index) => (
            <a
              ...
              key={item.id}
              className={`relative group cursor-pointer overflow-hidden ${index % 2 === 0 ? 'md:mt-12' : ''}`}
            >
              <div className="aspect-[4/5] bg-organic-gray relative overflow-hidden rounded-3xl">
                <img
                  src={item.img}
                  alt={item.client}
                ...
                {/* Overlay */}
                <div className="absolute inset-0 bg-organic-black/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center items-center text-center p-6 border-2 border-organic-cyan m-4 rounded-2xl">
