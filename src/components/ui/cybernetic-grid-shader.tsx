...
const CyberneticGridShader = () => {
    ...
    `;
      ...
      void main() {
        ...
        // base grid color pulsing (Cyan: 0.28, 0.89, 0.75)
        vec3 gridColor = vec3(0.28, 0.89, 0.75);
        vec3 color     = gridColor
                       * line
                       * (0.5 + sin(t * 2.0) * 0.2);

        // energetic pulses along grid (Purple: 0.65, 0.55, 0.98)
        float energy = sin(uv.x * 20.0 + t * 5.0)
                     * sin(uv.y * 20.0 + t * 3.0);
        energy = smoothstep(0.8, 1.0, energy);
        color += vec3(0.65, 0.55, 0.98) * energy * line;
        ...
        // transparency is set low here (alpha = 0.15) for a subtle background effect
